...
HAR uses the *hyperpolymath universal ABI/FFI standard* with
...
|*Linear* |Ephapax |Ownership semantics (supplementary)
|`+src/abi/*.eph+`
...
Manages connections to downstream automation targets (rpa-elysium,
etc.):

[width="100%",cols="59%,30%,11%",options="header",]
|===
...
|`+EventEnvelope linear+` |Every incoming event is either routed or
dead-lettered

|`+RouteDecision linear+` |Every routing decision dispatched to exactly
one target

|`+TargetConnection linear+` |Every connection explicitly disconnected
(no leaks)
|===

*Note:* When Idris2 and Ephapax conflict, Idris2 definitions are
authoritative.
...
|`+har_router_create+` |`+() -> c_int+` |Create router, returns slot or
-1
...
Both repos consume the same proven-servers ABI types with identical tag
values. Events flow: HAR (Producing) → Queue → rpa-elysium (Consuming).
