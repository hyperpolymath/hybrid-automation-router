...
sed -i 's/hybrid-automation-router/my-project/g' $(grep -rl 'hybrid-automation-router' .)
sed -i 's/hybrid-automation-router/MY_PROJECT/g' $(grep -rl 'hybrid-automation-router' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `hybrid-automation-router` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `hybrid-automation-router` | One-line description | `A tool for X` | flake.nix |
| `hybrid-automation-router` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `{{project}}` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `hybrid-automation-router` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
